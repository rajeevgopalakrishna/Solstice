...
#### 9. Exceptions ####
Error-handling in Solidity using *require*, *assert* and *revert* has some nuances as described [here](https://solidity.readthedocs.io/en/latest/control-structures.html#error-handling-assert-require-revert-and-exceptions) and so has to be used appropriately. For example, *require* should be used for input validation whereas *assert* should be used for invariant validation as part of internal error checking. Also, exceptions from *require* do not consume any gas but those from *assert* consume all the available gas. This analyser highlights the use of these constructs so that developers can validate these nuances.
...
* `-i <Input Solidity File>` Specify the input Solidity file. (Required)
* `-o <Output Report File>` Specify the output report file. Default is stdout. (Optional)
* `-d` Print debug statements. Helpful in debugging during development. (Optional)
* `-h` Print help message. (Optional)
* `--<AnalyserID to be run>` Specify the analyser to be run by IDs 1,2,..15. Default is to run all 15 analysers. (Optional)
